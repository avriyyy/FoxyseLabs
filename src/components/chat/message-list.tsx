  ...
  if (messages.length === 0) {
    return (
      <div className="flex-1 flex items-center justify-center overflow-y-auto">
        <div className="max-w-[720px] w-full text-center px-6 py-12">
          {/* Logo */}
          <div className="flex justify-center mb-8">
            <svg width="56" height="56" viewBox="0 0 32 32" fill="none">
              ...
              />
              <defs>
                <linearGradient id="msg-gradient" x1="0" y1="0" x2="32" y2="32" gradientUnits="userSpaceOnUse">
          ...
          {/* Welcome Text */}
          <h1
            className="text-3xl lg:text-[36px] font-bold mb-3 leading-tight"
            style={{ color: "var(--foreground-primary)", fontFamily: "var(--font-heading)" }}
          >
            What can I help you with?
          </h1>
          <p
            className="text-sm lg:text-base mb-8 leading-relaxed max-w-[480px] mx-auto"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-[560px] mx-auto">
            {PROMPT_SUGGESTIONS.flat().map((prompt) => (
              <button
                key={prompt}
                onClick={() => onPromptClick?.(prompt)}
                className="h-14 flex items-center px-4 text-sm text-left rounded-xl transition-all hover:border-[var(--accent-secondary)] hover:bg-white/[0.03]"
                style={{
          ...
          <div className="flex gap-3">
            ...
            >
              <div className="flex gap-1.5">
                <div className="w-1.5 h-1.5 rounded-full animate-bounce" style={{ background: "var(--foreground-muted)" }} />
                <div className="w-1.5 h-1.5 rounded-full animate-bounce [animation-delay:0.1s]" style={{ background: "var(--foreground-muted)" }} />
                <div className="w-1.5 h-1.5 rounded-full animate-bounce [animation-delay:0.2s]" style={{ background: "var(--foreground-muted)" }} />
              </div>
            </div>
          </div>
        )}
